...
def rebalance(buy_list, sell_list, pass_count)
  ...
  @portfolio.positions.each do |position|
    ...
    if open_orders.length == 0
      if sell_list.include?(symbol)
        ...
        @portfolio.sell(symbol, position["instrument"], quantity)
        ...
        if buy_count > 0
          ...
          @portfolio.buy(symbol, position["instrument"], buy_count)
        else
  ...
  non_owned_buy_list.each do |stock|
    ...
    if buy_count > 0
      ...
      @portfolio.buy(stock, @portfolio.instrument_for_symbol(stock), buy_count)
    else
